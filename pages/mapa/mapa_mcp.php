...
<script type="text/javascript" src="https://cdn.sheetjs.com/xlsx-0.20.3/package/dist/xlsx.full.min.js"></script>
    ...
    <style>
        ...
        .bottom-right-download-container {
                position: fixed;
                z-index: 1052;
                bottom: 20px;
                right: 5px;
...
<body>
    ...
    <div style="height: 100%;" leaflet-browser-print-content>
        ...
        <div id="contentMap">
            ...
            <div class="top-right-filters-container d-flex flex-column">
                <select class="form-control" style="width: 150px;" id="select-filter-service">
                    <option value="">Seleccione un servicio</option>
                    <option value="suministro_agua">Agua potable</option>
                    <option value="disposicion_basura">Saneamiento</option>
                    <option value="electricidad">Servicio electrico</option>
                </select>
                <select class="form-control" style="width: 150px;" id="select-filter-type">
...
<div id="modalDetalle" class="modal-detalle oculto">
    <div class="modal-detalle-backdrop"></div>

    <div class="modal-detalle-content">
        <div class="modal-detalle-header">
            <span>Detalle de Vivienda</span>
...
.modal-detalle-body{
    ...
    <script>
      ...
      }).setView([4.04047, -67.7001571], 15, false);
        ...
        map.addLayer(baseLayers.SateliteToponimia);
    ...
    const consultasAlmacenadas = {
            ...
            'disposicion_basura' : {
                    'saneamiento_ambiental': {
                        'nombreCapa' : 'Saneamiento Ambiental',
                        'consultas' : 'disposicion_basura="Servicio de Recoleccion"',
                        'tipo' : 'casas',
                        'icono' : '4/'+hexaColors[7]+'/#ffffff/solid/2'
                    },
                    'entierro': {
                        'nombreCapa' : 'Entierro',
                        'consultas' : 'disposicion_basura="Entierro"',
                        'tipo' : 'casas',
                        'icono' : '4/'+hexaColors[8]+'/#ffffff/solid/2'
                    },
                    'quema': {
                        'nombreCapa' : 'Quema',
                        'consultas' : 'disposicion_basura="Quema"',
                        'tipo' : 'casas',
                        'icono' : '4/'+hexaColors[9]+'/#ffffff/solid/2'
                    },
                    'al_aire_libre': {
                        'nombreCapa' : 'Al Aire Libre',
                        'consultas' : 'disposicion_basura="Al Aire Libre"',
                        'tipo' : 'casas',
                        'icono' : '4/'+hexaColors[10]+'/#ffffff/solid/2'
                    },
            },
            'electricidad' : {
                    'con_contrato': {
                        'nombreCapa' : 'Con Contrato',
                        'consultas' : 'electricidad="Con Contrato"',
                        'tipo' : 'casas',
                        'icono' : '4/'+hexaColors[11]+'/#ffffff/solid/2'
                    },
                    'sin_contrato': {
                        'nombreCapa' : 'Sin Contrato',
                        'consultas' : 'electricidad="Sin Contrato"',
                        'tipo' : 'casas',
                        'icono' : '4/'+hexaColors[12]+'/#ffffff/solid/2'
                    },
                    'sin_electricidad': {
                        'nombreCapa' : 'Sin Electricidad',
                        'consultas' : 'electricidad="Sin Electricidad"',
                        'tipo' : 'casas',
                        'icono' : '4/'+hexaColors[13]+'/#ffffff/solid/2'
                    },
            },
        };
       ...
       function obtener_puntos_almacenados(consulta, nombreCapa, tipo, perso, type = null) {

            // ===== ICONO =====
            const explode = perso.split('/');
            const ciTamano       = explode[0];
            const ciRellenoColor = explode[1];
            const ciLineaColor   = explode[2];
            const ciEstiloLinea  = explode[3];
            const ciAnchoLinea   = explode[4];

            const iconPersonalizado = iconPersonalizadoFunction(
                ciRellenoColor, '1', ciAnchoLinea, ciLineaColor, ciEstiloLinea, ciTamano
            );

            // ===== AJAX =====
            $.ajax({
                url: 'consulta_mcp.php',
                type: 'POST',
                dataType: 'json',
                data: {
                    consulta: consulta,
                    tipo: tipo,
                    tipo_unidad_inmobiliaria: type
                }
            })
            .done(function (resp) {
                ...
                const myLayer = L.geoJSON(resp.geojson, {
                    pointToLayer: function (feature, latlng) {
                        ...
                        return L.circleMarker(latlng).bindPopup(`
                            ...
                            <hr>
                            <a class="aVerDetallesPopup"
                            style="cursor:pointer"
                            onclick='verDetalleVivienda(${JSON.stringify(feature.data)})'>
                            <i class='fa fa-home'></i> Ver información completa
                            </a>
                        `);
                    },
                    style: iconPersonalizado
               ...
               // toast("success", "Se agregó una nueva capa");
                ...
                // ===== PROPIEDADES =====
                let tipoC = (tipo === 'persona') ? 'personas' : 'casas';
            ...
            .fail(function (err) {
                console.error(err);
                toast("error", "Error consultando el servidor");
            });
        }


        const etiquetasFicha = {

            // ===== RESPONSABLE =====
            nombreResponsable: "Nombre del Responsable",
            responsable: "Cédula Responsable",
            telefono: "Teléfono",
            // ===== IDENTIFICACIÓN =====
            id_vivienda: "Código de Vivienda",
            cod_catastro: "Código Catastral",
            cod_ine: "Código INE",
            ...
            material_construccion: "Material",
            condicion_vivienda: "Condición",
            cantidad_habitaciones: "Habitaciones",
            vivienda_venezuela: "Gran Misión Vivienda",
            bnbt: "BNBT",
            ...
            // ===== SERVICIOS =====
            agua_potable: "Suministro de Agua",
            suministro_agua_consumo: "Agua de Consumo",
            almacenamiento_agua: "Almacenamiento",
            tratamientoAgua: "Tratamiento del Agua",
            tapaPozo: "Tapa de Pozo",
            agua_servidas: "Aguas Servidas",
            disposicion_basura: "Disposición de Basura",
            ...
            robos: "Robos",
            cantidadRobos: "Cantidad de Robos",
            ultimoRobo: "Último Robo",
            denucio: "Denuncia",

            // ===== PRODUCTIVO =====
            actividad_productiva: "Actividad Productiva",
            bom_grandes: "Bombonas Grandes",
            bom_mediana: "Bombonas Medianas",
            bom_pequena: "Bombonas Pequeñas",
            equipos: "Equipos"
        ...
        const seccionesFicha = {
            ...
            "Datos de la Vivienda": [
                "tipo","tipoConstruccion","material_construccion","condicion_vivienda",
                "cantidad_habitaciones","familias","habitantes","tenencia_tierra",
                "vivienda_venezuela","bnbt"
            ],
            "Servicios": [
                "agua_potable","suministro_agua_consumo","almacenamiento_agua",
                "tratamientoAgua","tapaPozo","agua_servidas",
                "electricidad","medidor_electricidad",
                "telefonia","internet","television","tv_satelital",
                "disposicion_basura","frecuencia_recoleccion",
                "equipos"
            ],
            "Riesgo y Seguridad": [
                "zonaRiesgo","robos","cantidadRobos","ultimoRobo","denucio"
            ],
            "Actividad Productiva": [
                "actividad_productiva","bom_grandes","bom_mediana","bom_pequena","equipos"
            ]
            
        };

function verDetalleVivienda(data){

    let html = `<div class="ficha-catastral">`;

    for(const titulo in seccionesFicha){

        html += `<h3 class="ficha-titulo">${titulo}</h3>`;
        html += `<div class="ficha-grid">`;

        seccionesFicha[titulo].forEach(campo => {

            if(data[campo] !== undefined && data[campo] !== ""){

                html += `
                    <div class="ficha-item">
                        <span class="ficha-label">${etiquetasFicha[campo] || campo}</span>
                        <span class="ficha-valor">${data[campo]}</span>
                    </div>
                `;
            }
        });
...
function obtenerMarkersCapasActivas() {
    const datos = [];

    // Recorre todos los layers registrados en el control
    controlP._layers.forEach(obj => {

        // Solo si la capa está activa en el mapa
        if(obj.overlay && map.hasLayer(obj.layer)) {

            // obj.layer._layers contiene todos los markers de esa capa
            const layerGroup = obj.layer._layers;

            for(const key1 in layerGroup) {  // key dinámico
                const marker = layerGroup[key1];

                // Algunos layers pueden ser FeatureGroup anidados
                if(marker._layers) {
                    for(const key2 in marker._layers) {
                        const innerMarker = marker._layers[key2];
                        if(innerMarker.feature?.data) {
                            datos.push(innerMarker.feature.data);
                        }
                    }
                } else if(marker.feature?.data) {
                    datos.push(marker.feature.data);
                }
            }
        }
    });
...
function exportarMarkersVisiblesExcel(keys = null) {
    ...
    if(lista.length === 0){
        alert("No hay viviendas visibles en el mapa");
        return;
    }

    // Si no se indica keys, exporta todas las etiquetas de ficha
    const columnas = keys || Object.keys(etiquetasFicha);

    const filas = lista.map(data => {
        let obj = {};
        columnas.forEach(key => {
            obj[ etiquetasFicha[key] || key ] = data[key] ?? '';
        });
        return obj;
    });
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Viviendas Visibles");

    XLSX.writeFile(wb, `viviendas_mapa_${filas.length}.xlsx`);
}
...
document.querySelector('.modal-detalle-backdrop')
    .addEventListener('click', cerrarModalDetalle);
        ...
        function cargarRepresentaciones(service = null, type = null) {
            ...
            let consultasMostrar = consultasAlmacenadas;

            // 👉 Si viene servicio, solo usamos ese grupo
            if (service && consultasAlmacenadas[service]) {
                consultasMostrar = {
                    [service]: consultasAlmacenadas[service]
                };
            }

            // recorrer servicios
            $.each(consultasMostrar, function (keyServicio, capas) {
        ...
        function limpiarTodasLasRepresentaciones() {
            map.eachLayer(layer => {
                if (layer instanceof L.CircleMarker) {
                    map.removeLayer(layer);
                    // quitar del control
                    Object.values(controlP._layers).forEach(obj => {
                        if (!obj.overlay) return; // solo overlays
                        controlP.removeLayer(obj.layer);
                    });
                }
            });
        }
        ...
        $(document).ready(function() {
            ...
            $('#select-filter-service').on('change', function() {
                let service = $(this).val() || null;
                let type = $('#select-filter-type').val() || null;
                // quitar todas las representaciones
                limpiarTodasLasRepresentaciones();
                cargarRepresentaciones(service, type);
            });

            $('#select-filter-type').on('change', function() {
                let type = $(this).val() || null;
                let service = $('#select-filter-service').val() || null;
                limpiarTodasLasRepresentaciones();
                cargarRepresentaciones(service, type);
            });
        })
...
<script>
    ...
    <?php
    if ($tipo = '1') {
        $query66 = "SELECT * FROM consultas_almacenadas";
